                ...
                public void onTextFrame(String payload, boolean finalFragment, int rsv) {
                    ...
                    switch (type) {
                        case Constants.PULL_TRACE_DETAIL_TYPE:
                            List<String> wrongTraceIdList = JSON.parseObject("traceIdList",
                                    new TypeReference<List<String>>() {
                            });
                            Integer bucketPos = JSON.parseObject("bucketPos", Integer.class);
                            String wrongTraceDetails =
                                    ClientDataStreamHandler.getWrongTracing(wrongTraceIdList,
                                            bucketPos);
                            String msg = String.format("{\"type\": %d, \"data\": %s, " +
                                            "\"dataPos\": %d}",
                                    Constants.TRACE_DETAIL, wrongTraceDetails, bucketPos);
